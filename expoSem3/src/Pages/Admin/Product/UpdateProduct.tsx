...
function UpdateProduct() {
  ...
  useEffect(() => {
    ...
    const getProductDetails = async () => {
      try {
        ...
        setPicture(product.picture); // Assuming the product contains an image URL or base64 string
      } catch (error) {
    ...
    if (file) {
      ...
      try {
        const compressedImage = await compressor.compress(file, {
          quality: 0.5, // Reduce the quality for smaller file size
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(payload);
      await axios.put(`http://localhost:5205/api/product/${id}`, payload);
  ...
  return (
    <div className="d-flex">
      ...
      <div className="container-fluid px-4">
        <h1 className="h3 my-3">Update Product</h1>
